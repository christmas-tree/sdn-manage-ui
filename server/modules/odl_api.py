...
def _get_node(node_info):
    ...
    for host in list_host:
        if node_info == host.node_id or node_info == host.node_name or node_info == host.node_ip or node_info == host.node_mac:
            return host
    raise Exception(f"Cannot find node \"{node_info}\" in list of nodes: {list_switch} {list_host}")
...
def get_topo(forced=False):
    ...
    list_host = []
    list_links = []
    if os.path.isfile("sdntopo.json"):
        with open("sdntopo.json", "r") as f:
            data = json.load(f)
    else:
        data = ODLRequest.get(OPERATION_URL + '/network-topology:network-topology')
        with open("sdntopo.json", "w") as f:
...
def change_flow_id(switch):
    ...
    for idx, f in enumerate(list_flow):
        ...
        url = CONF_URL + "/opendaylight-inventory:nodes/node/" + switch.node_id + \
            "/flow-node-inventory:table/0/flow/" + urllib.parse.quote_plus(f['id'])
...
def add_flow(switch, inport, outport, src_mac, dst_mac):
    ...
    flow = Flow(flow_id, table_id, switch_id,
